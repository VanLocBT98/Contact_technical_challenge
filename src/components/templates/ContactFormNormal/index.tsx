...
export interface ContactFormProps {
  title?: string;
  description?: string;
  placeholders?: { [key in keyof ContactFormInput]?: string };
  submitText?: string;
  price: string;
}

const ContactFormNormal: React.FC<ContactFormProps> = ({
  title,
  description,
  submitText,
  placeholders,
  ...
  const method = useForm<ContactFormInput>({
    ...
    resolver: yupResolver(contactSchemas as any)
  });
  ...
  const handleSubmit = async (params: ContactFormInput) => {
    ...
    fetch(`${import.meta.env.VITE_API_URL}`, {
      ...
      body: JSON.stringify({ ...dataForm, Type: TYPE.NORMAL, Price: price })
    })
      .then((res) => res.json())
      .then((data) => {
        emailjs.sendForm(
  ...
  return (
    <div className='t-contactForm'>
      <div className='t-contactForm_wrap'>
        <Typography
          type='h3'
          modifiers={['32x48', 'deepSpaceSparkle', 'center', '700']}
          content={title}
        />
        {description && (
          <div className='t-contactForm_description'>
            <Typography
              type='p'
              modifiers={['14x21', 'darkLiver', 'center']}
              content={description}
            />
          </div>
        )}
      ...
      <Notify isOpen={isOpen} onClose={() => setIsOpen(false)} {...dataConfirm} />
    </div>
  );
};

export default ContactFormNormal;
